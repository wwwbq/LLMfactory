...
def _parse_args(parser: "HfArgumentParser", args: Optional[Dict[str, Any]] = None) -> Tuple[Any]:
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith((".yaml", ".json")):
        cfg_dict = Config().fromfile(os.path.abspath(sys.argv[1]))
        
        cfg_info = "\n" + "="*20 + " cfg " + "="*20 + "\n" + cfg_dict.pretty_text +"="*45
        logger.info(cfg_info)
        
        return parser.parse_dict(cfg_dict)
...
def set_hf_logger_format(formatter=logging.Formatter("[%(asctime)s][Hugging-Face][%(levelname)s] %(message)s")) -> None:
    from transformers.utils.logging import _get_library_root_logger
    # hf_logger = logging.getLogger(_get_library_name())
    
    handlers = _get_library_root_logger().handlers
    # handlers = hf_logger.handlers

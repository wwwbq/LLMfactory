...
class LogCallback(TrainerCallback):
    ...
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        logs = dict(
            ...
            percentage=f"{round(self.cur_steps / self.max_steps * 100, 2):.2f}%" if self.max_steps != 0 else 100,
    ...
    def parse_logs(self, logs:dict):
        logs = deepcopy(logs)
        
        if self.num_steps_per_epoch < 0:
            self.num_steps_per_epoch = int((logs["current_steps"] / logs["epoch"]) + 0.5)
        
        log_str, logs_item = "", []
        
        current_steps = logs.pop("current_steps")
        total_steps = logs.pop("total_steps")
        epochs = current_steps // self.num_steps_per_epoch
        total_epoch = logs.pop("total_epoch")
        
        logs_item.extend([
            f'cur_step: [{current_steps-int(epochs)*self.num_steps_per_epoch}/{self.num_steps_per_epoch}]',
            f'epoch: [{logs.pop("epoch"):.1f}/{total_epoch}]',
